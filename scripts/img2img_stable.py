...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="a painting of a virus monster playing guitar",
        help="the prompt which correctly describe original image",
    )
    ...
    pil_img = Image.open(opt.init_img).convert("RGB")
    noise_out = find_noise_for_image(model, pil_img, opt.desc)
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        z_enc = sampler.stochastic_encode(
                            init_latent, torch.tensor([t_enc] * batch_size).to(device), noise=noise_out
                        )
                        # decode it
